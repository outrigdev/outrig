...
function keydownWrapper(
    fn: (keyEvent: OutrigKeyboardEvent) => boolean
): (event: KeyboardEvent | React.KeyboardEvent) => void {
    return (event: KeyboardEvent | React.KeyboardEvent) => {
        const keyEvent = adaptFromReactOrNativeKeyEvent(event);
        const rtnVal = fn(keyEvent);
        if (rtnVal) {
...
function keyEventToKeyDesc(keyEvent: OutrigKeyboardEvent): string {
    let keyDesc: string[] = [];
    if (keyEvent.cmd) {
        keyDesc.push("Cmd");
    }
    ...
    if (keyEvent.key != null && keyEvent.key != "") {
        if (keyEvent.key == " ") {
            keyDesc.push("Space");
        } else {
            keyDesc.push(keyEvent.key);
        }
    } else {
        keyDesc.push("c{" + keyEvent.code + "}");
    }
    return keyDesc.join(":");
}
...
function isCharacterKeyEvent(event: OutrigKeyboardEvent): boolean {
...
function isInputEvent(event: OutrigKeyboardEvent): boolean {
...
function checkKeyPressed(event: OutrigKeyboardEvent, keyDescription: string): boolean {
...
function adaptFromReactOrNativeKeyEvent(event: React.KeyboardEvent | KeyboardEvent): OutrigKeyboardEvent {
    let rtn: OutrigKeyboardEvent = {} as OutrigKeyboardEvent;
...
function keyboardEventToASCII(event: OutrigKeyboardEvent): string {
    // check modifiers
